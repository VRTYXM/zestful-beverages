import React, { useEffect, useState } from 'react';
import { useDispatch, useSelector } from 'react-redux';
import { Link } from 'react-router-dom';

import { addItem, CartItem, selectCartItemsById } from '../../redux/slices/cartSlice';

const typeNames = ['Ориг. упаковка', 'Zestful'];

type ProductBlockProps = {
  id: string | number;
  title: string;
  price: number;
  imageUrl: string;
  sizes: number[];
  types: number[];
};

const ProductBlock: React.FC<ProductBlockProps> = ({
  id,
  title,
  price,
  imageUrl,
  sizes,
  types,
}) => {
  const dispatch = useDispatch();
  const cartItems = useSelector(selectCartItemsById(id));

  const initialType = types.length > 0 ? types[0] : -1;
  const [activeType, setActiveType] = useState(initialType);
  const [activeSize, setActiveSize] = useState(0);

  useEffect(() => {
    if (!types.includes(activeType)) {
      setActiveType(types.length > 0 ? types[0] : -1);
    }
  }, [activeType, types]);

  const addedCount = cartItems
    ? cartItems.reduce((accumulator, item) => accumulator + item.count, 0)
    : 0;

  const onClickAdd = () => {
    if (activeType === -1) {
      alert('Пожалуйста, выберите тип упаковки!');
      return;
    }

    const item: CartItem = {
      id,
      title,
      price,
      imageUrl,
      type: typeNames[activeType],
      size: sizes[activeSize],
      idForSearch: `${id}&${title}&${typeNames[activeType]}&${sizes[activeSize]}`,
      priceForPack: price * sizes[activeSize],
      count: 0,
    };
    dispatch(addItem(item));
  };

  return (
    <div className="product-block-wrapper">
      <div className="product-block">
        <Link to={`/product/${id}`}>
          <img className="product-block__image" src={imageUrl} alt="Product" />
          <h4 className="product-block__title">{title}</h4>
        </Link>
        <div className="product-block__selector">
          <ul>
            {types.length > 0 ? (
              types.map((type) => (
                <li
                  key={type}
                  onClick={() => setActiveType(type)}
                  className={activeType === type ? 'active' : ''}>
                  {typeNames[type]}
                </li>
              ))
            ) : (
              <li className="disabled">Нет доступных типов упаковки</li>
            )}
          </ul>
          <ul>
            {sizes.map((size, i) => (
              <li
                key={size}
                onClick={() => setActiveSize(i)}
                className={activeSize === i ? 'active' : ''}>
                {size} шт.
              </li>
            ))}
          </ul>
        </div>
        <div className="product-block__bottom">
          <div className="product-block__price">{price} ₽ / шт.</div>
          <button
            onClick={onClickAdd}
            className="button button--outline button--add"
            disabled={activeType === -1}>
            <svg
              width="12"
              height="12"
              viewBox="0 0 12 12"
              fill="none"
              xmlns="http://www.w3.org/2000/svg">
              <path
                d="M10.8 4.8H7.2V1.2C7.2 0.5373 6.6627 0 6 0C5.3373 0 4.8 0.5373 4.8 1.2V4.8H1.2C0.5373 4.8 0 5.3373 0 6C0 6.6627 0.5373 7.2 1.2 7.2H4.8V10.8C4.8 11.4627 5.3373 12 6 12C6.6627 12 7.2 11.4627 7.2 10.8V7.2H10.8C11.4627 7.2 12 6.6627 12 6C12 5.3373 11.4627 4.8 10.8 4.8Z"
                fill="white"
              />
            </svg>
            <span>Добавить</span>
            {addedCount > 0 && <i>{addedCount}</i>}
          </button>
        </div>
      </div>
    </div>
  );
};

export default ProductBlock;
